from database_managment.crud import create, delete_by_params, update_by_params
...
class CRUDService(ABC):
    ...
    async def read_list(self, *table_params, **filters):
        db_raw_data = await self.select_list_coroutine(self.conn, *table_params, **filters)
        return [self.interfaces_factory.create_output_list_model_object(row) for row in db_raw_data]

    async def read_detail(self, **filters):
        db_raw_data = await self.select_detail_coroutine(self.conn, **filters)
        return self.interfaces_factory.create_output_model_object(db_raw_data)

    async def create(self, input_model_object, **extra_fields):
        db_model_object = self.interfaces_factory.create_db_model_object(input_model_object, **extra_fields)
    ...
    async def delete(self, **filters):
        db_model_object = self.interfaces_factory.db_model()
